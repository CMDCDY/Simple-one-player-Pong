...
slider = tkinter.Scale(t, from_=8, to=25, orient='horizontal', length=100, bg='green', label="speed")
...
def ballmove():
  ...
  while ballrun % 2 != 0:
      ...
      if playball.x < 120 and playball.y in range((int(ball1.y) - 60), int(ball1.y) + 60):
          ...
          rscored()
      if int(playball.x) > int(width) - 100 and int(playball.y) in range((int(ball2.y) - 60), int(ball2.y) + 60):
...
def rdown2(event):
    t5 = threading.Thread(target=rup, name='t5')
    t5.start()


def rup2(event):
    t6 = threading.Thread(target=rdown, name='t6')
    t6.start()
...
t4.start()
t7 = threading.Thread(target=binding3, name='t7')
t7.start()
